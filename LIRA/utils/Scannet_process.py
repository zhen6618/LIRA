...
def mask_to_polygons(mask):
    # Find the contours in the mask and use cv2.RETR_CCOMP to process multi-layer structures (multi-object segmentation)
    contours, hierarchy = cv2.findContours(mask.astype(np.uint8), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
    
    polygons_list = []

    for i, contour in enumerate(contours):
        # Only process the outermost contours or nested segmented regions
        if hierarchy[0][i][3] == -1:
            # Convert the contour points directly to COCO format, that is, flatten them into [x1, y1, x2, y2, ..., xn, yn]
            points = contour.reshape(-1, 2).tolist()
            flattened_points = [coord for point in points for coord in point]  
            
            # Add to COCO format segmentation list
...
# Define the sort order for classes and colors
...
# Create a sorting dictionary of classes and colors to provide weights when sorting
class_order = {cls: i for i, cls in enumerate(classes)}
color_order = {color: i for i, color in enumerate(colors)}

instructions_num = 360  # The number of instructions per scene
            ...
            for _, d in enumerate(data):
                ...
                if candidate_len == 0:
                    ...
                    for kk in range(candidate_len):
                        ...
                        if candidate_data[kk]['mask'].shape[0] != 968 * 1296:
                            candidate_data[kk]['mask'] = candidate_data[kk]['mask'].reshape(480, 640)  # The mask of some scenes is 480x640
                        else:
                            candidate_data[kk]['mask'] = candidate_data[kk]['mask'].reshape(968, 1296)

                    # Sort by class, then sort the elements of the same class by color
                ...
                if candidate_len == 0:
                    ...
                    for c in range(candidate_len):
                        points.append(mask_to_polygons(sorted_candidate_data[c]['mask']))  # Already sorted
