...
def prepare_ins(instance_volume, semantic_volume):
    ...
    semantic_dict = {
        ...
        11: 'picture', 12: 'counter', 13: 'desk', 14: 'curtain', 15: 'refrigerator',
        16: 'shower curtain', 17: 'toilet', 18: 'sink', 19: 'bathtub', 20: 'otherfurniture',
        }
    ...
    unique_ins = np.unique(instance_volume)
    # print("unique_ins: ", unique_ins)
    unique_sem = np.unique(semantic_volume)
    ...
    for ins in unique_ins:
        ...
        key = str(int(new_ins)) + ' ' + sem_label
        semnatic_positions[key] = np.round(ins_center, decimals=2).tolist()
    ...
    # print("     ")
    # print("semantic_ids: ", semantic_ids)
    # print("     ")
    # print("semantic_classes: ", semantic_classes)
    # print("     ")
    # print("pos: ", pos)
    print("     ")
    print("semnatic_positions: ", semnatic_positions)

    return instance_ids, semantic_ids, semantic_classes, pos
...
def ins_to_llm(instance_volume, semantic_volume):
    ...
    return semnatic_positions

def select_target_instances(instance_volume, semantic_volume, target_instances):

    instance_volume[instance_volume != target_instances] = 0
    instance_volume[instance_volume == target_instances] = 1

    semantic_volume[instance_volume != target_instances] = 0
    semantic_volume[instance_volume == target_instances] = 1

    return instance_volume, semantic_volume
...
class SaveScene(object):
    ...
    def tsdf_ins2mesh(voxel_size, origin, tsdf_vol, semantic_vol, instance_vol):
        ...
        color_palette = [
            [192, 192, 192], [255, 0, 0], [0, 255, 0], [0, 0, 255], [255, 255, 0], [0, 255, 255],
            [255, 0, 255], [128, 0, 0], [0, 128, 0], [0, 0, 128], [128, 128, 0],
            [0, 128, 128], [128, 0, 128], [255, 128, 0], [128, 255, 0], [0, 255, 128],
            [0, 128, 255], [128, 0, 255], [255, 0, 128], [128, 128, 128], [255, 255, 255],
        ]  # RGB
        # color_palette = [
    ...
    def save_incremental(self, epoch_idx, batch_idx, imgs, outputs):
        ...
        if (tsdf_volume == 1).all():
            ...
            # Marching cubes
            mesh, mesh_semantic, mesh_instance = self.tsdf_ins2mesh(self.cfg.MODEL.VOXEL_SIZE, origin, tsdf_volume, semantic_volume, instance_volume)
            # save
