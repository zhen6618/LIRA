...
from .modules import keep_largest_connected_region_3d, evaluate_instance_segmentation, keep_connected_region_3d
...
class Fusion(nn.Module):
    ...
    def get_local_ins_infos(self, ins_info, relative_origin, voxel_size):
        ...
        if 'masks' in ins_info['gt']:
            ...
            match_gt_ins_ids[match_gt_iou < 0.6] = 1000
    ...
    def LLM(self, scale, instruction, voxel_size):
        ...
        scan_results_path = 'scan_results_train/'
    ...
    def forward(self, coords_target=None, tsdf_target=None, inputs=None, batch_id=None, do_ins_fusion=False, scale=2, outputs=None, save_mesh=False, ins_infos=None, qa_instruction=None):
        ...
        if not do_ins_fusion:  # 如果不进行实例融合
            ...
            global_instance = keep_connected_region_3d(vis_coords, global_instance.to(vis_coords.dtype), n=2)  # 连通域滤波去除噪声

            save_path=f'results/debug_global_fused_mesh_{self.scene_name[scale]}_{self.instruction_id[scale]}.ply'
            label2mesh(vis_coords+5, 
                    tsdf.squeeze(1), 
                    vis_scope, 
                    global_instance.unsqueeze(1), 
