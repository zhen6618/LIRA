...
class Conv3d(nn.Module):
    """ Sparse 3D Submifold Conv (with Residual Connection) """
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, num_layers):
        ...
        for n, k in zip([input_dim] + h, h + [output_dim]):
            self.layers.append(SparseConv3d_Residual(dim_in=n, dim_out=k, Kernel=3))

    def forward(self, x, coords, spitial_shape):
        coords = torch.cat([torch.zeros_like(coords[:, 0]).unsqueeze(1), coords], dim=1)  # [N, 4(bxyz)]
        
        for layer in self.layers:
            x = layer(x=x, coords=coords, spitial_shape=spitial_shape, bs=1)
        return x

class SparseConv3d_Residual(nn.Module):
    def __init__(self, dim_in, dim_out, Kernel):
        super(SparseConv3d_Residual, self).__init__()
        self.SConv3d = SparseSubMConv3d(dim_in, dim_out, Kernel)
