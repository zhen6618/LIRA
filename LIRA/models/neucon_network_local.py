...
class NeuConNet(nn.Module):
    ...
    def forward(self, inputs, outputs):
        ...
        if len(batch_indexs) > 0:
            "reason seg multi-batch parallel reasoning [b*9, 3, 480, 640]"
            with torch.no_grad():
                # t_rs_start = time.time()
                # masks: [N_mask, 1024, 1024], rs_text_feats: [N_mask, 4096], rs_img_feats: [N_mask, 256, 1024, 1024]
                response_all, masks_all, rs_text_feats_all, rs_img_feats_all = self.reason_seg_model.inference_batch_parallel(batch_indexs, prompt_all, rs_imgs_np_all) 
                # print("average rs inference time: ", (time.time() - t_rs_start) / len(batch_indexs), "s.")
            ...
            # if len(batch_indexs) != bs:
            #     print("len(batch_indexs) != bs")
            batch_index_count = -1
            for batch in range(bs):
                ...
                for rs_i in range(len(selected_img_ids)):  # reason_seg_imgs: [9, 3, 480, 640]
                    batch_rs_i = batch_index_count * len(selected_img_ids) + rs_i
                    ...
                    ins_volume = []  # Each voxel corresponds to an instance category [num_ins, num_voxels]
                    ...
                    # vis
                    if len(ins_volume) > 0:
                        label2mesh(up_coords_batch[:, 1:], tsdf_target_all[batch], tuple(self.cfg.N_VOX), ins_volume[0].float().unsqueeze(1), f'results/label_target_{batch_rs_i}.ply')
        ...
        # label2mesh(up_coords_batch[:, 1:], tsdf_target_all[batch], tuple(self.cfg.N_VOX), ins_masks.float().unsqueeze(1), f'results/label_target_fusion.ply')
...
If the number of labels exceeds 20, the colors are cycled from the beginning.
    ...
    # print(f"PLY save to {output_file}!")
