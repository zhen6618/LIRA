...
def generate_all_instance_mask():
    ...
    with open(scene_file, 'r') as split_file:
        for scene_name_i, scene_name in enumerate(split_file):
            ...
            # if not scene_name in ['scene0438_00']:
            #     continue

            if os.path.exists(root_path + f'grounding_scene_mask_infos_83/{scene_name}_2d_mask_data.json.gz'):
            ...
            with gzip.open(root_path + f'grounding_scene_mask_infos_83/{scene_name}_2d_mask_data.json.gz', "wt", encoding="utf-8") as save_f:  # 压缩
...
def generate_VLM_grounding_data_from_mask():
    ...
    mode = 'train'  # base数据集 mode 都是 train   
    scene_file = f'generate_color_label/base_scenes_val.txt'  
    ...
    with open(scene_file, 'r') as split_file:
        for scene_name_i, scene_name in enumerate(split_file):
            ...
            for instructions_i in range(instructions_num):
                ...
                # if os.path.exists(root_path + f'grounding_scene_rs_infos_base/{scene_name}_{instructions_i}_reasoning_seg_data.json.gz'):
                #     print(f'{scene_name}_{instructions_i}', 'exists')
                #     continue
                ...
                with open(root_path + f'grounding_scene_qa_infos_base_new/{scene_name}_{instructions_i}_qa.pkl', 'rb') as file:
                    scene_qa = pickle.load(file)
                ...
                if  valid_scene_flag == True:               
                    ...
                    #     pickle.dump(grounding_2d_data, file)
                    #     print("Done")
                    with gzip.open(root_path + f'grounding_scene_rs_infos_base_new/{scene_name}_{instructions_i}_reasoning_seg_data.json.gz', "wt", encoding="utf-8") as file:
