...
import numpy as np
from neu_utils import coordinates
import transforms3d
import torch
...
class ResizeImage(object):
    ...
    def __init__(self, size, selected_img_ids):
        self.size = size
        self.selected_img_ids = selected_img_ids

    def __call__(self, data):
        repeat_num = int(9 / len(self.selected_img_ids))
        imgs_copy = [item for item in data['imgs'] for _ in range(repeat_num)]

        for i, im in enumerate(imgs_copy):
            ...
            if i in self.selected_img_ids:
                data['imgs'][self.selected_img_ids.index(i)] = np.array(im, dtype=np.float32)
...
class RandomTransformSpace(object):
    ...
    def transform(self, data, transform=None, old_origin=None,
        ...
        for i in range(data['depth'].shape[0]):  # N_views
            size = data['depth'][i].shape
        ...
        if 'tsdf_list_full' in data.keys():
            ...
            for l, (tsdf_s, rgb_s, semantic_s, instance_s) in enumerate(zip(data['tsdf_list_full'], data['rgb_list_full'], data['semantic_list_full'], data['instance_list_full'])):
