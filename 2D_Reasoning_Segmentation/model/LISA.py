        ...
        **kwargs,
        ...
        if inference:
            ...
            for i in range(n_batch):
                ...
                input_ids_empty = input_ids[0][:start_idx_empty + window_size_empty]

                output_empty = self.generate(
                    images=images_clip, # [N, 3, 224, 224]
                    input_ids=input_ids_empty.unsqueeze(0),  # [N, 69]
                    max_new_tokens=512,  # 512
                    num_beams=1,
                    output_hidden_states=True,
                    return_dict_in_generate=True,
                )
                output_empty_ids = output_empty.sequences
                seg_token_mask_empty = output_empty_ids[:, 1:] == self.seg_token_idx
                has_seg_empty = seg_token_mask_empty.sum() > 0
        ...
        if inference:  # validate
            IMAGE_TOKEN_INDEX = -200
            return {
                "pred_masks": pred_masks,
                "gt_masks": gt_masks,
                "has_seg_empty": has_seg_empty,
                "input_ids": input_ids[0][input_ids[0] != IMAGE_TOKEN_INDEX],
                "output_empty_ids": output_empty_ids[0][output_empty_ids[0] != IMAGE_TOKEN_INDEX],
            }
