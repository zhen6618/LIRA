...
def mask_to_polygons(mask):
    # Locate the contours in the mask and use cv2.RETR_CCOMP to handle the multi-layer structure (multi-object segmentation)
    contours, hierarchy = cv2.findContours(mask.astype(np.uint8), cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
    
    polygons_list = []

    for i, contour in enumerate(contours):
        # Only handle the outermost contour or nested segmented regions
        if hierarchy[0][i][3] == -1:
            # Directly convert the contour points to the COCO format, that is, flatten them into [x1, y1, x2, y2,..., xn, yn]
            points = contour.reshape(-1, 2).tolist()
            flattened_points = [coord for point in points for coord in point]  # Flatten the dots into a single list
            
            # Add to the split list in COCO format
...
root_path = '/root/paddlejob/workspace/env_run/zhouzhen05/Data_SSD3/scannet_grounding/reason_seg/Scannet_2D_Seg_hard/'
...
# Create a sorting dictionary for classes and colors to provide weights during sorting
class_order = {cls: i for i, cls in enumerate(classes)}
color_order = {color: i for i, color in enumerate(colors)}

instructions_num = 10  The number of instructions for each scenario is base: 100, hard: 10

# Define the method for calculating the center of mask
def mask_center(mask):
    indices = np.argwhere(mask)
    center = indices.mean(axis=0)  # Calculate the center point
    return tuple(center)
...
with open(read_path, 'r') as split_file: 
    for scene_name_i, scene_name in enumerate(split_file):
        ...
        for instructions_i in range(instructions_num):
            ...
            if not os.path.exists(root_rs_path + f'grounding_scene_rs_infos_hard/{scene_name}_{instructions_i}_reasoning_seg_data.json.gz'):
                print(scene_name, 'not exists')
                continue        

            with gzip.open(root_rs_path + f'grounding_scene_rs_infos_hard/{scene_name}_{instructions_i}_reasoning_seg_data.json.gz', "rt", encoding="utf-8") as file:
                data = json.load(file)
            ...
            for _, d in enumerate(data):
                ...
                if candidate_len == 0:
                    ...
                    for kk in range(candidate_len):
                        ...
                        if candidate_data[kk]['mask'].shape[0] != 968 * 1296:
                            candidate_data[kk]['mask'] = candidate_data[kk]['mask'].reshape(480, 640) # The mask for some scenes is 480x640
                        else:
                            candidate_data[kk]['mask'] = candidate_data[kk]['mask'].reshape(968, 1296)

                    # Sort by class order, and then sort the elements within the same class by color order
                ...
                if candidate_len == 0:
                    ...
                    for c in range(candidate_len):
                        points.append(mask_to_polygons(sorted_candidate_data[c]['mask']))  # The order has been arranged
                ...
                # image_re.save(root_path + 'train/' + all_info['image'])     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  Remember to resize the image to 1024x1024 in LISA !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
